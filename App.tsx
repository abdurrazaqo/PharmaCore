...
import { PowerSyncProvider } from './src/lib/powersync';

// Robust, dependency-free Markdown renderer for AI Consult
  ...
  return (
    <>
      {text.split('\n').map((line, lineIndex) => {
        ...
        // Advanced regex to split by bold (** or __)
        ...
        return (
          <div 
            key={lineIndex} 
            className={`
              ${isHeader ? "font-bold text-slate-900 dark:text-white mt-4 mb-2 text-base" : "mb-1.5 leading-relaxed text-slate-700 dark:text-slate-200"}
              ${isListItem ? "pl-5" : ""}
            `}
          >
            {parts.map((p, i) => {
                      ...
                      <div className="flex flex-col h-full">
                        <div className="bg-primary p-5 text-white flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[10px] opacity-70 mt-1 uppercase tracking-widest font-bold">OpenFDA • RxNorm • Groq AI</p>
                          </div>
                          <button 
                        ...
                        <div className="flex-1 overflow-y-auto p-5 bg-slate-50 dark:bg-slate-900/50">
                          {aiResponse ? (
                            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 rounded-2xl text-sm leading-relaxed text-slate-700 dark:text-slate-200 shadow-sm overflow-hidden">
                              <FormattedAiText text={aiResponse} />
